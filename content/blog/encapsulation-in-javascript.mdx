...
- **No direct access** → Properties or methods shouldn't be changed from outside arbitrarily
- **Controlled access** → Private data can only be read or updated via defined interfaces (e.g., getters, setters)
- **Prevents misuse** → Stops invalid or unexpected data manipulations
...
const p = new Person("Görkem");
console.log(p.getName()); // Görkem
p.setName("Ahmet");
...
const person = createPerson("Görkem");
console.log(person.getName()); // Görkem
...
✅ Enforced consistency and rules

### Drawbacks

❌ Slightly more boilerplate (e.g., getters and setters)  
❌ Historically required workarounds before ES2022
...
Start applying encapsulation in your JavaScript projects today, and watch your code become more robust, maintainable, and secure! 🔒
